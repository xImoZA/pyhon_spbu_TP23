...
def get_input_matrix(str_matrx, column_matrx):
    ...
    for lines in range(1, str_matrx + 1):
        input_str = list(
            map(int, input(f"Введите {lines} строку матрицы через пробел: ").split())
        )
        ...
        while len(input_str) != column_matrx:
            input_str = list(
                map(
                    int,
                    input(
                        f"Введено некорректное число элементов. Попробуйте снова. "
                        f"\nВведите {lines} строку матрицы через пробел: "
                    ).split(),
                )
            )
...
def angle_between_vectors(vector_value_1, vector_value_2):
    return (
        acos(
            scalar_product_vectors(vector_value_1, vector_value_2)
            / (vector_length(vector_value_1) * vector_length(vector_value_2))
        )
        * 180
        / pi
    )
...
def scalar_product_vectors(vector_value_1, vector_value_2):
    return sum(
        [vector_value_1[i] * vector_value_2[i] for i in range(len(vector_value_1))]
    )
...
def sum_of_matrices(matrix1, matrix2):
    return [
        [
            matrix1[score_str][score_column] + matrix2[score_str][score_column]
            for score_column in range(len(matrix1[0]))
        ]
        for score_str in range(len(matrix1))
    ]
...
def scalar_product_of_matrices(matrix1, matrix2):
    return [
        [
            sum([matrix1[x][i] * matrix2[i][y] for i in range(len(matrix1[0]))])
...
def operation_matrix_124(parameter):
    ...
    if parameter == "1":
        ...
        return "\n".join(
            " ".join(map(str, i))
            for i in transposition_matrix(get_input_matrix(str_marix, columns_matrix))
        )
    elif parameter == "2":
        ...
        return "\n".join(
            " ".join(map(str, i))
            for i in sum_of_matrices(input_matrix_1, input_matrix_2)
        )
        ...
        return "\n".join(
            " ".join(map(str, i))
            for i in product_of_matrix_and_number(input_matrix, number)
        )


def operation_matrix_3():
    str_marix_1 = int(
        input("Для работы с матрицей введите количество строк первой матрицы: ")
    )
    ...
    return "\n".join(
        " ".join(map(str, i))
        for i in scalar_product_of_matrices(input_matrix_1, input_matrix_2)
    )


if __name__ == "__main__":
    type_v_m = get_correct_input(
        "Для того чтобы работать с векторами введите V, чтобы работать с матрицами введите M: ",
        ["V", "M"],
    )

    if type_v_m == "V":
        operation = get_correct_input(
            "Выберите операцию:\n 1)нахождение скалярного произведения \n "
            "2)вычисление длины вектора \n 3)нахождение угла между векторами \n "
            "Выбранная операция (1/2/3): ",
            ["1", "2", "3"],
        )
